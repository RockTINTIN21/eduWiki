...
const data: UniversityData = {
  aboutUniversity: {
    ...
    description: 'Университет был основан в 1808 году как Белградская высшая школа. 27 февраля 1905 года королевским указом был преобразован в университет. Интенсивный рост университета наблюдался после Второй мировой войны.\n' +
  ...
  requirements: [
    ...
    {title: 'Документы для поступления', text: ['Аттестат 9/11 классов', 'Паспорт', 'Диплом', 'Медицинская справка', 'Рекомендательное письмо']},
    {title: 'Сроки подачи документов', text: 'До 28 декабря'},
    {title: 'Обязательность страховки / финансовой гарантии', text: 'От 2000 €'},
  ],
  programs: [
    {title: 'Информатика и IT', id: '1234',
      programs: [
        {name: 'Buisness and Management', id: '14145'},
        {name: 'Buisness and Management', id: '14140'},
      ]
    },
    {title: 'Информатика и IT', id: '1232',
      programs: [
        {name: 'Buisness and Management', id: '14144'},
        {name: 'Buisness and Management', id: '14147'},
      ]
    },
    {title: 'Информатика и IT', id: '1233',
      programs: [
        ...
        <div className='md:w-[80%] flex flex-col gap-6'>
          <section id='0' data-section='0'>
            ...
            <div className='flex flex-col gap-3'>
              ...
              style={{ backgroundImage: `url(${data.aboutUniversity.bgImage})` }}
              >

                <div className='flex justify-between'>
                  <div className='flex gap-2 bg-[#F3F3F3] px-4 py-1 rounded-full w-max items-center'>
                    <span className='font-medium'>{data.aboutUniversity.name}</span>
                    <Image src={`https://flagsapi.com/${data.aboutUniversity.regionCode}/flat/64.png`}
                ...
                <div className='flex gap-4 ms-auto'>
                  <Button className='px-7' variant='secondary'>Учусь здесь</Button>
                </div>
              </div>
          ...
          <section id='2' data-section='2'>
            ...
            <div className='flex flex-col gap-3'>
              {data.requirements.map((requirement, i) => (
                <div className='bg-secondary rounded-2xl flex flex-col md:flex-row justify-between md:justify-start gap-2 p-4' key={i}>
                  <span className='text-[#5a5a5a] leading-5 md:w-1/2'>{requirement.title}</span>
                  {Array.isArray(requirement.text) ?
                    <div className='flex gap-2 flex-wrap md:w-1/2'>
                      {requirement.text.map((item, i) =>
                        <p key={i} className='text-md border-black border-[1px] rounded-md px-2'>{item}</p>
                      )}
                    </div>
                    : <p className='font-medium text-md md:1/2'>{requirement.text}</p>}
            ...
            <Tabs items={[
              {
                ...
                label: 'Бакалавр',
                children: 
                  <Accordion key={data.programs[0].id} tabs={[
                    {
              ...
              },
              {
                ...
                label: 'Магистратура',
                children:
                  <Accordion key={data.programs[1].id} tabs={[
                    {
              ...
              },
              {
                ...
                label: 'Докторантура',
                children:
                  <Accordion key={data.programs[2].id} tabs={[
                    {
