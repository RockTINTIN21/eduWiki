...
const PromoCard = ({
	...
	return (
      ...
      `}
		>
			<HugeiconsIcon
				className={isShortCard === false ? "text-accent" : "text-white"}
			...
			<div className={isShortCard === false ? "flex flex-col gap-2 pb-12" : ""}>
				<p
					className={`font-medium ${isShortCard === false ? "text-xl" : " text-md"}`}
				>
					{title}
				</p>
				<span
					className={` whitespace-pre-line ${isShortCard === false ? "text-md" : "text-sm"}`}
				>
					{description}
				</span>
			</div>
